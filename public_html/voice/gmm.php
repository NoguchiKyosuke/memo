...
def train_gmm(data_label, features, train_keys, train_folders, audio_ext, dict_file, ncomp, init_only=False):
    ...
    return gmm
        ...
        <div class="explanation-block">
            ...
            <ul>
                <li>72から102行目: GMMの訓練を効率的に行うように事前学習モデルのパラメータを使用して最初のトレーニングをしている。</li>
                ...
                <li>92行目: Xを使用して最初のGMMの初期化を行っている。このときのパラメータとして、n_componentsは混合するガウス分布の数、covariance_type='diag'は共分散行列の成約タイプが対角共分散、max_iter=200はEMアルゴリズムの試行回数を200回に設定、verbose=2は各ステップの対数尤度と所要時間を表示する。<sup><a href="#parameter">[2]</a></sup>。このとき、対角共分散が使われる理由として、音声特徴量の抽出にLFCCを使っているため特徴量同士で無相関のため、対角共分散が適用されやすいためである。<sup><a href="#slide">[3]</a></sup></li>
                <li>99行目: GMMの初期化が完了したことをログに記録している。</li>
                <li>102行目: pickleを使用して訓練したモデルをダンプしている。次の訓練からはこのモデルを使用して訓練を行う。</li>
                <li>107行目から166行目: EMアルゴリズムを実行している。<sup><a href="#slide">[3]</a></sup></li>
                <li>109行目: EMアルゴリズムを10回繰り返すループを記述している。</li>
                <li>110行目から114行目: トレーニングが途中で止まった場合、それまでトレーニングしていた部分を効率的に学習を行えるような処理を行っている。</li>
                <li>116行目: 各ガウスコンポーネントにおける信頼度や重みの割当を代入する配列nk_accをゼロリセットしている。</li>
                <li>117行目: 新たな平均を求める際に使用するための、特徴ベクトルの重み付き和を計算している。</li>
                <li>118行目: 新たな共分散を求める際に使用するための、特徴ベクトルの二乗の重み付き和を計算している。</li>
                <li>119行目: </li>
            </ul>
        </div>
    </section>
